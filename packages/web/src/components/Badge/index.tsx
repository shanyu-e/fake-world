...
const Badge = ({ type = "number", style, text, className, children, hidden }: PropsWithChildren<Props>) => {
  if (hidden || (isNil(text) && type === "number") || text === 0) {
    return <>{children}</>;
  }
  ...
  return (
    <div className="relative">
      {children}
      {type === "dot" && <div className={twMerge("-right-1 -top-1 absolute h-[10px] w-[10px] rounded-full bg-red-500", className)} style={style} />}
      {type === "number" && (
        <div
          className={twMerge(
