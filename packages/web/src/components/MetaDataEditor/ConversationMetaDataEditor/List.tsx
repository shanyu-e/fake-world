...
const ConversationListMetaDataEditor = ({ index }: EditorProps<unknown, IStateProfile["id"]>) => {
  ...
  const onFinish = (values: TConversationItem) => {
    setConversationList((prev) => {
      let maxDialogueId: number;
      if (!prev) {
        maxDialogueId = 1;
      } else {
        maxDialogueId = Math.max(...prev.map((item) => Number(item.dialogueId))) + 1;
      }
      // 找出prev对应的id
      const id = prev.find((item) => item.dialogueId === "1".toString())?.id;
      // 根据类型处理不同的字段
      let newItem: TConversationItem;
      if (values.type === EConversationType.voice) {
        // 语音类型需要确保 duration 是 string
        const voiceValues = values as IConversationTypeVoice;
        newItem = {
          ...voiceValues,
          id,
          dialogueId: maxDialogueId.toString(),
          duration: typeof voiceValues.duration === "string" ? voiceValues.duration : String(voiceValues.duration || "0"),
