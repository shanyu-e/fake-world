...
// 数据源工厂
export function createDataSource<T>(key: string, endpoint: string, customConfig?: Partial<DataSourceConfig>): IDataSource<T> {
...
export const dataSources = {
  // 用户档案
  profiles: (get: Getter) => createDataSource<any>("profiles", "/profiles", get(dataSourceConfigAtom)),

  // 钱包（单个）
  wallet: (get: Getter) => createDataSource<any>("wallet", "/wallet", get(dataSourceConfigAtom)),

  // 所有钱包
  allWallets: (get: Getter) => createDataSource<any>("allWallets", "/wallet", get(dataSourceConfigAtom)),

  // 对话列表
  dialogues: (get: Getter) => createDataSource<any>("dialogues", "/dialogues", get(dataSourceConfigAtom)),

  // 聊天记录
  conversations: (get: Getter) => createDataSource<any>("conversations", "/conversations", get(dataSourceConfigAtom)),

  // 朋友圈
  feeds: (get: Getter) => createDataSource<any>("feeds", "/feeds", get(dataSourceConfigAtom)),

  // 交易记录
  transactions: (get: Getter) => createDataSource<any>("transactions", "/transactions", get(dataSourceConfigAtom)),
