...
import { allProfilesAtom, allProfilesErrorAtom, allProfilesLoadingAtom, createProfile, loadAllProfiles } from "@/stateV2/profileV2";
import {
  allWalletsAtom,
  allWalletsErrorAtom,
...
export const DataSourceTest: React.FC = () => {
  ...
  const handleCreateProfile = async () => {
    try {
      ...
      // createProfile 已经自动更新了本地状态，不需要再次加载
    } catch (error) {
          ...
          {!allWalletsLoading && !allWalletsError && allWallets && Array.isArray(allWallets) && (
            <div className="space-y-2">
              ...
              <div className="max-h-60 overflow-y-auto">
                ...
                {allWallets.length > 10 && <div className="text-gray-500">...还有 {allWallets.length - 10} 个钱包</div>}
              </div>
            </div>
          )}
        </div>
          ...
          {!profilesLoading && !profilesError && profiles && Array.isArray(profiles) && (
            <div className="space-y-2">
              ...
              <div className="max-h-40 overflow-y-auto">
                ...
                {profiles.length > 5 && <div className="text-gray-500">...还有 {profiles.length - 5} 个用户</div>}
              </div>
            </div>
          )}
        </div>
