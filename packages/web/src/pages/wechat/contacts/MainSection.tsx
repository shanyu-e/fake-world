...
import { allProfilesAnchorDataAtom, loadAllProfiles } from "@/stateV2/profileV2";
...
const MainSection = () => {
  ...
  const [stuckInfo, setStuckInfo] = useState<Map<string, boolean>>(() => {
    // 安全地初始化 stuckInfo，如果 anchorData 是 Promise 则使用空 Map
    if (anchorData instanceof Map) {
      return getStuckInfo(anchorData);
    }
    return new Map();
  });
  ...
  // 当 anchorData 更新时，同步更新 stuckInfo
  useUpdateEffect(() => {
    if (anchorData instanceof Map) {
