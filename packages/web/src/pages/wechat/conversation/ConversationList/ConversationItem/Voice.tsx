  ...
  return (
    <>
      <CommonBlock
        ...
        blockStyle={{
          width: `${(Number(duration) || 0) <= 10 ? 30 + (Number(duration) || 0) * 2.2 : 52}%`,
          maxWidth: "52%",
        }}
        extraElement={
          !isRead &&
      ...
      >
        <div className={twJoin("flex h-6", role === "mine" ? "flex-row-reverse" : "flex-row")}>
          <VoiceSVG width={28} height={28} className={twJoin("self-center", role === "mine" && "rotate-180")} />
          <span className="self-center text-sm">{duration}&apos;&apos;</span>
        </div>
      </CommonBlock>
      {showStt && !!stt && (
        <CommonBlock hideAvatar upperText={undefined} senderId={senderId} innerBlockClassName="bg-white" blockClassName="!mt-1">
          {stt}
        </CommonBlock>
      )}
    </>
