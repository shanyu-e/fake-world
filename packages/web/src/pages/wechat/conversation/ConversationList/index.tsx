...
const ConversationList = () => {
  const { listRef, conversationId, sendTransfer, sendRedPacketAcceptedReply } = useConversationAPI();
  ...
  const mappedSortableListData = useMemo(() => {
    // Ensure conversationList is an array before calling .map() to prevent potential runtime errors
    // if it happens to be undefined or null, addressing the lint warning.
  ...
  const generateTransferReplyConversation = (conversationItemId: TConversationItem["id"]) => {
    const item = (getConversationListValueSnapshot(conversationId) ?? []).find((v) => v.id === conversationItemId);
    const { amount, originalSender, role } = item as IConversationTypeTransfer;
    setConversationList((prev) =>
      prev.map((item) => (item.id === conversationItemId ? ({ ...item, transferStatus: "accepted" } as TConversationItem) : item))
    );
    sendTransfer({
      amount,
  ...
  const generateRedPacketAcceptedReplyConversation = (conversationItemId: TConversationItem["id"]) => {
    setConversationList((prev) =>
      prev.map((item) => (item.id === conversationItemId ? ({ ...item, redPacketStatus: "accepted" } as TConversationItem) : item))
    );
    ...
    >
      ...
      >
        {conversationList?.map((item) => {
          ...
          return (
            <canBeDetected.div
              className={twJoin("group flex flex-col space-y-4", item.role, isEdit && "cursor-grab")}
              key={item.id}
              metaData={[
                {
                  type: EMetaDataType.ConversationItem,
                  index: [conversationId, item.id],
                  treeItemDisplayName: (data) => `消息（${ConversationTypeLabel[data.type]}${data.role ? `-${data.role}` : ""}）`,
                  operations,
                  label: "单个消息",
                },
                {
