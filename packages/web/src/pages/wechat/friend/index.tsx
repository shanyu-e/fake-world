...
import { dialogueListAtom, type IDialogueItem, type TStateDialogueList } from "@/stateV2/dialogueList";
import { type IStateProfile, PRIVACY_TEXT_MAP } from "@/stateV2/profile";
...
const Friend = () => {
  ...
  const { avatarInfo, gender, nickname, wechat, hideGender, remark, area, phone, tags, privacy, thumbnailInfo, hideThumbnail, description } = profile;
      ...
      <div className="flex flex-1 flex-col overflow-auto bg-[rgba(237,237,237,1)]">
            ...
            {!isEmpty(phone) && (
              <div className="flex border-black/5 border-b bg-white pl-5">
                ...
                <div className="flex flex-1 flex-col">
                  {phone?.map((v, i) => (
                    <div className="cursor-pointer border-black/5 border-b py-3 text-wechatLink-1 last:border-none" key={i}>
                      {v}
                    </div>
                  ))}
                </div>
        ...
        <div className="mb-12">
          <div className="mt-2 cursor-pointer bg-white">
            {!hideThumbnail && (
              <div className="ml-5 flex items-center border-black/5 border-b py-3" onClick={() => navigate(`/wechat/moments/user/${userId}`)}>
          ...
          <div className="mt-2 cursor-pointer bg-white">
            <div
              className={twJoin("flex items-center justify-center space-x-2 py-3 text-wechatLink-1", !isMyself && "border-black/5 border-b")}
              onClick={() => {
                if (userId) {
                  navigate(`/wechat/conversation/${userId}`);
                  setDialogueList((prev: TStateDialogueList) => {
                    if (prev.some((v: IDialogueItem) => v.friendId === userId)) return prev;
                    return [
                      {
                        id: nanoid(5),
                        friendId: userId,
